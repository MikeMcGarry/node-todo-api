...
var deleteDocuments = (db, collection, callback, payload=false) => {
  ...
  var result = collection.findOneAndDelete({_id: new ObjectID('59c4d0904f9251070473f29c')}).then((result) => {
...
var operation = (collection, operationFunc, callback, payload=false, url2=url) => {
  ...
  var insertDocuments = (db, collection, payload, callback) => {
    var collection = db.collection(collection)
...
operation("Todos", deleteDocuments, handler)
